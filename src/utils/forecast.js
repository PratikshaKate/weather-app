...
const forecast = (latitude, longitude, callback) => {
  ...
  request({ url, json: true }, (error, response) => {
    if (error) {
      ...
      callback(
        undefined,
        `The temperature is ${response.body.current.temperature} but it feel like it is ${response.body.current.feelslike}. The weather is currently ${response.body.current.weather_descriptions[0]}`
      );
    }
  });
};
